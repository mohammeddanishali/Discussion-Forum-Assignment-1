...
public class UserActivity {
	...
	public void seeAllQuestions(UserActivity userActivity, User user) throws NumberFormatException, IOException {
		...
		// ** Please write your code below: **
		//
		// Replace the ... in the if statement condition with correct boolean statement.
		...
		//
		// You can check how many the number of questions by using the size() function
		// on the questions variable
		// such as: questions.size()
		//
		// The code for printing out "No question posted yet" and information about the
		// question
		// has been written for you. You simply need to replace the ... in the if
		// statement condition.
	...
	public void deleteQuestion(UserActivity userActivity, User user) throws NumberFormatException, IOException {
		...
		// ** Please write your code below: **
		//
		// Note: This is the most challenging part of the project, so please don't worry
		// if you don't get it correct
		// on the first try. Please feel free to look at the solution if you are stuck.
		//
		// Write the logic to delete a question from the class forum, and you should
		// follow the rules below:
		// 1) If the current user's role is an admin, the user can delete the question
		//
		// 2) Else if the current user's role is a moderator, the user can delete the
		// question only:
		// a) If the user who wrote the question is a regular user
		// b) Or the current user is also the user who wrote the question
		// c) Otherwise, you should print: "You are not authorised to delete this
		// question"
		//
		// 3) Else if the current user is also the user who wrote the question, we can
		// delete the question.
		// Otherwise, you should print: "You are not authorised to delete this question"
		//
		// ** A few tips and pointers: **
		// 1) For a question, you can use question.getUser() to get the user who wrote
		// the question
		//
		...
		//
		// 6) You can user questionService.deleteQuestion(question) to delete the
		// question
		...
		else if (user.getUserRole() == UserRole.MODERATOR) { // the user is a moderator
			if (question.getUser().getUserRole() == UserRole.USER) {
				questionService.deleteQuestion(question); // deleting a question
			} else if (question.getUser().getUserRole() == user.getUserRole()) {
				questionService.deleteQuestion(question); // deleting a question
			} else {
				System.out.println("You are not authorised to delete this question"); // Print you cannot delete the
																						// question
			}
		}

		// Write else if condition to check if the user is a regular user
		else if (user.getUserRole() == UserRole.USER) { // the user is a normal user
			if (question.getUser().getUserRole() == user.getUserRole()) {
				questionService.deleteQuestion(question); // deleting a question
			} else {
				System.out.println("You are not authorised to delete this question"); // Print you cannot delete the
																						// question
