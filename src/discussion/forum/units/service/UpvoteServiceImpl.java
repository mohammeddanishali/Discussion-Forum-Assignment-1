...
public class UpvoteServiceImpl implements UpvoteService {
	...
	private Upvote getUpvote(User user, Question question, Reply reply) {
		for (Upvote upvote : upvotes) {
			...
			/*
			 * Change the below if condition such that user can not upvote the same question
			 * or same reply twice, but can upvote multiple questions and replies
			 */
			// if ((upvote.getUser() == user) && ((upvote.getQuestion() == question) ||
			// (upvote.getReply() == reply))) {
